...
class AppleActivity : AppCompatActivity() {
    ...
    //화면이 꺼졌을때 음악의 재생을 멈추는 로직 - 생명주기 onPause() -> 화면이 꺼졌을때 실행되는 구간이다.
        ...
        inner class ViewHolder(itemView: View) : RecyclerView.ViewHolder(itemView) {
            ...
            init {
                ...
                play.setOnClickListener {
                    ...
                    try {
                        //첫번째 음악을 키고 두번째 음악을 틀었을때 첫번째 음악이 꺼지고 두번째 음악이 나오는 로직
                        mediaPlayer?.stop()
                        mediaPlayer?.release() // 핸드폰의 리소스를 많이 차지하게 되므로 차지하는 것을 놓아준다.
                        mediaPlayer = null
