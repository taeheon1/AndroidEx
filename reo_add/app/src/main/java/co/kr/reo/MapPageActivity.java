...
public class MapPageActivity extends AppCompatActivity
        implements OnMapReadyCallback, ActivityCompat.OnRequestPermissionsResultCallback, ClusterManager.OnClusterItemClickListener<MyOffice> {
    ...
    ArrayList<OfficeDTO> officeList = new ArrayList();

    private List<String> list;
    ClusterManager<MyOffice> clusterManager;
    RecyclerView cluster_item_list;
    ...
    public void onMapReady(final GoogleMap googleMap) {
        ...
        call.enqueue(new Callback<ArrayList<OfficeDTO>>() {
            ...
            public void onResponse(Call<ArrayList<OfficeDTO>> call, Response<ArrayList<OfficeDTO>> response) {
                ...
                if (response.isSuccessful()) {
                    ...
                    Runnable runnable = new Runnable() {
                        ...
                        public void run() {
                            for (OfficeDTO office : officeList) {
                                try {
                                    addrList = geocoder.getFromLocationName(office.getOff_stdAddr(), 1);
                                    list.add(office.getOff_stdAddr());
                                    double x = addrList.get(0).getLatitude();
                                    double y = addrList.get(0).getLongitude();
                                    MyOffice officeItem = new MyOffice(x, y, office.getOff_name());
                                    clusterManager.addItem(officeItem);
                                } catch (Exception e) {
                                    e.printStackTrace();
                                }
                            }
                    ...
                    mHandler.postAtTime(runnable,0);
        ...
        clusterManager.setOnClusterClickListener(
                new ClusterManager.OnClusterClickListener<MyOffice>() {
                    @Override
                    public boolean onClusterClick(Cluster<MyOffice> cluster) {
                        ArrayList<MyOffice> cluster_items = new ArrayList<>(cluster.getItems());
                        ArrayList<OfficeDTO> recycler_office = new ArrayList<>();

                        for (int i = 0; i < cluster_items.size(); i++) {
                            for (int j = 0; j < officeList.size(); j++) {
                                if (cluster_items.get(i).getTitle().equals(officeList.get(j).getOff_name())) {
                                    recycler_office.add(officeList.get(j));
                                }
                            }
                        }
                        ClusterAdapter cluster_adapter = new ClusterAdapter(recycler_office, LayoutInflater.from(context), glide, activity);
                        cluster_item_list = findViewById(R.id.cluster_item_list);
                        cluster_item_list.setLayoutManager(new LinearLayoutManager(context, LinearLayoutManager.HORIZONTAL, false));
                        cluster_item_list.setAdapter(cluster_adapter);
                        cluster_item_list.setVisibility(View.VISIBLE);
        ...
        mMap.setOnMapClickListener(new GoogleMap.OnMapClickListener() {
            ...
            public void onMapClick(LatLng latLng) {
                if (cluster_item_list != null) {
                    cluster_item_list.setVisibility(View.INVISIBLE);
                }
...
//        locationText.setText("Latitude" + address.getLatitude() + "Longitude" + address.getLongitude() + "\n" + addressText);
    ...
    @Override
    public boolean onClusterItemClick(MyOffice myOffice) {
        Log.d("itemname", myOffice.getTitle());
        if (cluster_item_list != null) {
            cluster_item_list.setVisibility(View.INVISIBLE);
        }
        ArrayList<OfficeDTO> recycler_office = new ArrayList<>();
        ...
        for (int j = 0; j < officeList.size(); j++) {
            if (myOffice.getTitle().equals(officeList.get(j).getOff_name())) {
                recycler_office.add(officeList.get(j));
            }
        }
